...
def apply_classifier_modify(x, model, img, im0):
    # Apply a second stage classifier to YOLO outputs
    # Example model = torchvision.models.__dict__['efficientnet_b0'](pretrained=True).to(device).eval()
    im0 = [im0] if isinstance(im0, np.ndarray) else im0
    traffic_light_cls_shift = 80
        ...
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):
                if pred_cls1[j] == 9:
                    ...
                    im_tensor = (torch.Tensor(im).unsqueeze(0)).to(d.device)  # [1, 3, 224, 224]

                    pred_cls2 = model(im_tensor).argmax(1)  # classifier prediction

                    x[i][j][5] = pred_cls2 + traffic_light_cls_shift
